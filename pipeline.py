...
def run():
    global _incidents, _last_run, _is_running
    ...
    print(f"\n{'='*55}")
    print(f"  🌍 EarthWatch India — Fetching Live Data")
...
def start_auto_refresh(interval_minutes=5):
    """Runs pipeline every X minutes in background thread."""
    def loop():
        while True:
            try:
                run()
